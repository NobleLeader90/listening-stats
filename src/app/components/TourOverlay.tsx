...
interface TourOverlayProps {
  ...
  onPrev: () => void;
  onEnd: () => void;
  abortSignal: React.MutableRefObject<{ cancelled: boolean }>;
}
...
export function TourOverlay({
  ...
  const updatePosition = useCallback(() => {
    ...
    if (!el) {
      // Target not found -- skip to next step only if not cancelled
  ...
  useEffect(() => {
    ...
    // Lock scroll during the settle period
    const unlockScroll = lockScroll();

    // Wait for scroll to settle, then re-measure after paint
    const timer = setTimeout(() => {
      if (abortSignal.current.cancelled) {
        unlockScroll();
        return;
      }
      // Re-measure after paint to catch any layout shift from scroll
